...
def data_receive():
    ...
    moviedata_url = "https://api.themoviedb.org/3/movie/now_playing?language=ko-KR"
    headers = {"Authorization": f"Bearer {moviedata_token}"} # Bearer 토큰 방식으로 API 키 전달
    ...
    moviedata_url = "https://api.themoviedb.org/3/movie/upcoming?language=ko-KR"
    headers = {
    ...
    # 중복 제거 (title 기준)
    unique_titles = {item['title']: item for item in data}.values()
